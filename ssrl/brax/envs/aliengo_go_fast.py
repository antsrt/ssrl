...
class ControlCommand:
    ...
    @dataclasses.dataclass
    class ActionScale:
        """Scaling factors for different action components."""
        foot: float = 0.1
        phase: float = np.pi
        body_height: float = 0.05
        inv_gait_period: float = 0.5
    
    # Foot positions relative to base in local frame
    foot_positions: jp.ndarray
    # Phase for gait generation (from -π to π)
    phase: float
    # Desired body height from ground
    body_height: float
    # Inverse of gait period (frequency)
    inv_gait_period: float
    # Target joint positions
    q_des: jp.ndarray = None
    # Target joint velocities
    qd_des: jp.ndarray = None
    # Proportional gains
    Kp: jp.ndarray = None
    # Derivative gains
    Kd: jp.ndarray = None
    # Contact state for each foot
    contact: jp.ndarray = None
    # Leg phases
    leg_phases: jp.ndarray = None
    # Desired foot positions
    pdes: jp.ndarray = None
...
class AliengoGoFast(RlwamEnv):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.obs_limits = self.obs_limits.at[self._q_idxs, 0].set(
            jp.tile(AliengoUtils.LOWER_JOINT_LIMITS, 4) - 0.25)
        self.obs_limits = self.obs_limits.at[self._q_idxs, 1].set(
            jp.tile(AliengoUtils.UPPER_JOINT_LIMITS, 4) + 0.25)
    ...
    def _reward_normalized(self, obs: jp.ndarray,
        ...
        rew_torque_limits = self._barrier_sigmoid(
            u,
            -.9*AliengoUtils.MOTOR_TORQUE_LIMIT,
            .9*AliengoUtils.MOTOR_TORQUE_LIMIT,
            w=0.2
        )
        ...
        penalty_max = jax.lax.select(u > AliengoUtils.MOTOR_TORQUE_LIMIT,
                                     -(u - AliengoUtils.MOTOR_TORQUE_LIMIT + 1),
                                     -jp.exp(u-AliengoUtils.MOTOR_TORQUE_LIMIT))
        penalty_min = jax.lax.select(u < -AliengoUtils.MOTOR_TORQUE_LIMIT,
                                     (u + AliengoUtils.MOTOR_TORQUE_LIMIT - 1),
                                     -jp.exp(-u-AliengoUtils.MOTOR_TORQUE_LIMIT))
    ...
    def raw_reward_components(self, norm_obs: jp.ndarray,
        ...
        buf = 2.0
        torque_limit_over = jp.where(u > AliengoUtils.MOTOR_TORQUE_LIMIT - buf,
    ...
    def low_level_control_hardware(self, action: jp.ndarray,
        ...
        pdes, contact, leg_phases = AliengoGait.control(
        ...
        # inverse kinematics
        q_des = AliengoUtils.inverse_kinematics_all_legs(pdes)
        q_des = jp.clip(q_des,
                        jp.tile(AliengoUtils.LOWER_JOINT_LIMITS, 4),
                        jp.tile(AliengoUtils.UPPER_JOINT_LIMITS, 4))
        qd_des = jp.zeros((12,))
    ...
    def _limit_Kp(self, obs, q_des, qd_des, Kp, Kd):
        ...
        Kp = jp.where(
            torque_theo > AliengoUtils.MOTOR_TORQUE_LIMIT,
            (AliengoUtils.MOTOR_TORQUE_LIMIT - Kd*(qd_des - qd))/q_err, Kp)
        Kp = jp.where(
            torque_theo < -AliengoUtils.MOTOR_TORQUE_LIMIT,
            (-AliengoUtils.MOTOR_TORQUE_LIMIT - Kd*(qd_des - qd))/q_err, Kp)
        return Kp
